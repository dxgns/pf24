    ...
    <section className="rounded-2xl border border-white/10 bg-slate-950/35 p-5">
      ...
      <div className="mt-4 space-y-2">
        {lines.map((line, index) => (
          <p key={`${title}-${index}`} className="font-mono text-sm leading-6 text-slate-300">
            {line}
          </p>
        ))}
      </div>
...
export default function PilotModuleContent({ moduleNumber }: { moduleNumber: number }) {
  if (moduleNumber === 1) {
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-sky-400/20 bg-sky-400/5 px-4 py-3 font-mono text-sm text-sky-200">
          EGKK 121350Z 23012KT 9999 FEW030 17/10 Q1015
        </div>
        ...
        <ImageCard
          src="/academy/piloto/pe/metar.webp"
          alt="Ejemplo visual del METAR EGKK 121350Z 23012KT 9999 FEW030 17/10 Q1015"
          width={700}
          height={119}
          caption="Ejemplo visual de los grupos que componen un METAR."
        />
  ...
  if (moduleNumber === 2) {
    return (
      <div className="space-y-6">
        <Paragraph>La altimetría es el sistema que permite conocer la altitud de la aeronave. Se basa en la presión atmosférica, que disminuye con la altura. Un ajuste incorrecto del altímetro puede provocar errores en la altitud real, lo que representa un riesgo de separación con otras aeronaves o el terreno.</Paragraph>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <div key={feet} className="rounded-xl border border-white/10 bg-slate-950/35 p-4">
              <div className="text-xs text-slate-500">{feet}</div>
              <div className="mt-1 font-mono text-lg font-bold text-sky-300">{level}</div>
            </div>
          ))}
        </div>
  ...
  if (moduleNumber === 3) {
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-amber-400/20 bg-amber-400/5 px-4 py-3 text-sm font-semibold text-amber-200">
          Nunca debe ingresar a la pista sin autorización explícita.
        </div>

        <Subheading>Alfabeto fonético</Subheading>
        <ImageCard
          src="/academy/piloto/pe/alfabeto-fonetico.webp"
          alt="Alfabeto fonético ICAO de Alfa a Zulu"
          width={600}
          height={291}
          caption="Alfabeto fonético utilizado en comunicaciones aeronáuticas."
        />

        <Subheading>Ejemplo carta de rodaje</Subheading>
        <ImageCard
          src="/academy/piloto/pe/carta-rodaje-mdst.webp"
          alt="Carta de rodaje de MDST Santiago con pista 11/29 y calles Alfa y Bravo"
          width={700}
          height={394}
          caption="Ejemplo de carta de rodaje de Santiago / MDST."
        />
      </div>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-6">
            <div className="font-mono text-lg font-bold text-sky-300">VFR</div>
            <div className="mt-1 text-xs uppercase tracking-[0.15em] text-slate-500">Visual Flight Rules</div>
            <p className="mt-4 text-sm leading-7 text-slate-300">El piloto mantiene referencia visual con el terreno y otras aeronaves, volando solo en condiciones meteorológicas adecuadas.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-6">
            <div className="font-mono text-lg font-bold text-sky-300">IFR</div>
            <div className="mt-1 text-xs uppercase tracking-[0.15em] text-slate-500">Instrument Flight Rules</div>
            <p className="mt-4 text-sm leading-7 text-slate-300">La navegación y control de la aeronave se realizan principalmente con instrumentos, incluso sin visibilidad exterior.</p>
          </div>
        </div>
      </div>
    );
  ...
  if (moduleNumber === 5) {
    return (
      <div className="space-y-6">
        ...
        <ImageCard
          src="/academy/piloto/pe/circuito-transito.webp"
          alt="Diagrama de circuito de tránsito con viento en cara, viento cruzado, viento en cola, base y final"
          width={593}
          height={351}
          caption="Tramos principales de un circuito de tránsito aeronáutico."
        />
  ...
  if (moduleNumber === 6) {
    return (
      <div className="space-y-6">
        ...
        <Paragraph>En el campo ‘Ruta’ se debe colocar <span className="font-mono font-bold text-sky-300">LCL</span> (Local), indicando que se trata de un vuelo en circuito visual.</Paragraph>
        <Subheading>Ejemplo</Subheading>
        <ImageCard
          src="/academy/piloto/pe/plan-vuelo-local.webp"
          alt="Ejemplo de plan de vuelo VFR local con ruta LCL"
          width={199}
          height={200}
          caption="Ejemplo de datos de un plan de vuelo VFR local."
          compact
        />
      </div>
    );
  ...
  if (moduleNumber === 9) {
    return (
      <div className="space-y-6">
        ...
        <Paragraph>Su objetivo es transmitir información de manera clara, breve y sin ambigüedades. Un piloto PE debe conocer las frases básicas necesarias para realizar rodajes, despegues y notificaciones de posición en el circuito.</Paragraph>
        ...
        <Subheading>Ejemplo de comunicaciones</Subheading>
        <RadioSection
          title="Prueba de radio / contacto inicial con superficie"
          lines={[
        ...
        />
        <RadioSection
          title="Autorización de plan de vuelo"
          lines={[
        ...
        />
        <RadioSection
          title="Puesta en marcha y retroceso"
          lines={[
        ...
        />
        <RadioSection
          title="Torre — autorización de despegue"
          lines={[
        ...
        />
        <RadioSection
          title="Aterrizaje y salida de pista"
          lines={[
        ...
        />
        <RadioSection
          title="En superficie — rodaje a plataforma"
          lines={[
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-sky-400/20 bg-sky-400/5 p-6">
        <p className="mono text-xs uppercase tracking-[0.2em] text-sky-300">Evaluación y habilitación PE</p>
        <p className="mt-3 text-sm leading-7 text-slate-300">Este módulo corresponde a la etapa final del temario y se habilita únicamente después de haber visto los módulos anteriores.</p>
      </div>
    </div>
  );
}
