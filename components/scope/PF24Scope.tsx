...
type ConnectForm = { callsign: string; facility: FacilityCode | ""; rating: string; server: string; password: string; discordName: string; robloxName: string; info4: string };
...
const FACILITY_INFO: Record<FacilityCode, string> = { DEL: "AUTORIZACIONES/DELIVERY", GND: "SUPERFICIE/GROUND", TWR: "TORRE/TOWER", APP: "APROXIMACION/APPROACH", CTR: "CENTRO/CENTER" };
const FACILITY_LABELS: Array<{ value: FacilityCode; label: string }> = [
  { value: "DEL", label: "Delivery" }, { value: "GND", label: "Ground" }, { value: "TWR", label: "Tower" }, { value: "APP", label: "Approach" }, { value: "CTR", label: "Center" },
];
...
const RUNWAYS: Array<{ airport: string; runway: string }> = [
  ...
  { airport: "EGKK", runway: "26L" }, { airport: "EGKK", runway: "08R" }, { airport: "GCLP", runway: "03L" }, { airport: "GCLP", runway: "03R" },
  { airport: "GCLP", runway: "21L" }, { airport: "GCLP", runway: "21R" }, { airport: "LCLK", runway: "22" }, { airport: "LCLK", runway: "04" },
  { airport: "LCPH", runway: "11" }, { airport: "LCPH", runway: "29" }, { airport: "LCRA", runway: "10" }, { airport: "LCRA", runway: "28" },
  { airport: "LEMH", runway: "19" }, { airport: "LEMH", runway: "01" }, { airport: "MDAB", runway: "11" }, { airport: "MDAB", runway: "29" },
  { airport: "MDCR", runway: "12" }, { airport: "MDCR", runway: "30" }, { airport: "MDPC", runway: "26" }, { airport: "MDPC", runway: "27" },
...
const DEFAULT_RUNWAY_STATE: RunwayState = Object.fromEntries(RUNWAYS.map(({ airport, runway }) => [`${airport}-${runway}`, { active: false, dep: false, arr: false }]));
...
export default function PF24Scope({ initialPlans }: Props) {
  ...
  const [metarStation, setMetarStation] = useState("");
  ...
  const [runwayState, setRunwayState] = useState<RunwayState>(DEFAULT_RUNWAY_STATE);
  ...
  useEffect(() => {
    const stored = localStorage.getItem("pf24_scope_window_layout_v3"); if (stored) { try { setWindows({ ...DEFAULT_WINDOWS, ...JSON.parse(stored) }); } catch {} }
    const storedProfile = localStorage.getItem(PROFILE_STORAGE_KEY); if (storedProfile) { try { const p = JSON.parse(storedProfile) as Partial<StoredProfile>; setConnectForm((c) => ({ ...c, password: typeof p.password === "string" ? p.password : "", discordName: typeof p.discordName === "string" ? p.discordName : "", robloxName: typeof p.robloxName === "string" ? p.robloxName : "" })); } catch {} }
    const storedSettings = localStorage.getItem(SCOPE_SETTINGS_STORAGE_KEY); if (storedSettings) { try { const settings = JSON.parse(storedSettings) as { zoom?: number }; if (typeof settings.zoom === "number" && Number.isFinite(settings.zoom)) setScopeZoom(Math.max(25, Math.min(175, Math.round(settings.zoom)))); } catch {} }
    const storedRunways = localStorage.getItem(RUNWAY_STORAGE_KEY); if (storedRunways) { try { setRunwayState({ ...DEFAULT_RUNWAY_STATE, ...JSON.parse(storedRunways) }); } catch {} }
    setSettingsLoaded(true);
  }, []);
  useEffect(() => { localStorage.setItem("pf24_scope_window_layout_v3", JSON.stringify(windows)); }, [windows]);
  useEffect(() => { localStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify({ password: connectForm.password, discordName: connectForm.discordName, robloxName: connectForm.robloxName })); }, [connectForm.password, connectForm.discordName, connectForm.robloxName]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      const results = await Promise.all(stations.map(async (station): Promise<[string, MetarEntry]> => {
        ...
        } catch { return [station, { raw: null, loading: false, error: true }]; }
      }));
  ...
  useEffect(() => {
    const move = (e: MouseEvent) => { if (!dragRef.current) return; const { key, dx, dy } = dragRef.current; const mouseX = e.clientX / scopeScale; const mouseY = e.clientY / scopeScale; const maxX = Math.max(0, logicalWidth - WINDOW_WIDTHS[key] - 4); const maxY = Math.max(0, logicalHeight - 170); setWindows((cur) => ({ ...cur, [key]: { ...cur[key], x: Math.min(maxX, Math.max(0, mouseX - dx)), y: Math.min(maxY, Math.max(0, mouseY - dy)) } })); };
    const up = () => { dragRef.current = null; }; window.addEventListener("mousemove", move); window.addEventListener("mouseup", up); return () => { window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); };
  }, [scopeScale, logicalWidth, logicalHeight]);
  useEffect(() => {
    const load = async () => { const { data } = await supabase.from("atc_sessions").select("*").eq("is_active", true); setSessions((data ?? []) as ATCSession[]); }; load(); const ch = supabase.channel("scope-atc-sessions-ui").on("postgres_changes", { event: "*", schema: "public", table: "atc_sessions" }, load).subscribe(); return () => { supabase.removeChannel(ch); };
  }, []);
  useEffect(() => {
    const ch = supabase.channel("scope-flight-plans-ui").on("postgres_changes", { event: "*", schema: "public", table: "flight_plans" }, (payload) => { const next = payload.new as ScopeFlightPlan; const old = payload.old as ScopeFlightPlan; if (payload.eventType === "INSERT" && next.status !== "FINISHED") setPlans((c) => [next, ...c]); if (payload.eventType === "UPDATE") setPlans((c) => next.status === "FINISHED" ? c.filter((p) => p.id !== next.id) : c.some((p) => p.id === next.id) ? c.map((p) => p.id === next.id ? next : p) : [next, ...c]); if (payload.eventType === "DELETE") setPlans((c) => c.filter((p) => p.id !== old.id)); }).subscribe(); return () => { supabase.removeChannel(ch); };
  }, []);
  ...
  function executeCommand(raw: string) { const input = raw.trim(); if (!input) return; setConsoleLines((c) => [...c.slice(-5), input]); const [cmd, ...args] = input.split(/\s+/); const upper = args.map((a) => a.toUpperCase()); if (cmd === ".help") setConsoleLines((c) => [...c.slice(-5), ".fpl .metar .hdg .alt .clear"]); else if (cmd === ".metar") { const s = upper[0] || "MDST"; void loadMetar(s); } else if (cmd === ".clear") setConsoleLines([]); setCommand(""); }
  ...
  function showWindow(key: WindowKey) { setWindows((c) => ({ ...c, [key]: { ...c[key], open: true, collapsed: false } })); }
  function confirmConnect() { const callsign = connectForm.callsign.trim().toUpperCase(); if (!ATC_FREQUENCIES[callsign] || !callsignMatchesFacility(callsign, connectForm.facility)) return; setPosition(callsign); setConnected(true); setShowConnectDialog(false); }
  ...
  function openRunwaySelector() { setShowRunwaySelector(true); setToolStates((c) => c.map((v, i) => i === 0 ? true : v)); }

  const stripTime = now.toISOString().slice(11, 19);
  void plans; void sessions;
  return <main className="fixed left-0 top-0 overflow-hidden bg-[#151515] font-mono text-[12px] text-[#d8d8d8] select-none" style={{ width: `${100 / scopeScale}%`, height: `${100 / scopeScale}%`, transform: `scale(${scopeScale})`, transformOrigin: "top left" }}>
    <header className="absolute inset-x-0 top-0 z-50 h-[44px] border-b border-[#202426]">
      <div className="flex h-[21px] items-stretch bg-[#064a40] text-[#e2e2e2]">
        ...
        <div className="scopeTopGap w-[21px]"/><button className="scopeTopCell w-[24px] text-[6px] leading-[6px]">OPEN<br/>SCT</button><div className="scopeTopCell w-[52px] text-[10px] tracking-[.3px]">{stripTime}</div><div className="scopeTopGap w-[21px]"/><button className="scopeTopCell w-[24px] text-[6px] leading-[6px]">QUICK<br/>SET</button>
        <button onClick={openRunwaySelector} className={`scopeTopCell w-[25px] ${toolStates[0] ? "scopeToolOn" : ""}`}><ToolbarGlyph kind="screen" active={toolStates[0]}/></button>
        <div className="scopeTopGap w-[21px]"/><button onClick={() => toggleTool(1)} className={`scopeTopCell w-[25px] ${toolStates[1] ? "scopeToolOn" : ""}`}><ToolbarGlyph kind="vector" active={toolStates[1]}/></button><button onClick={() => toggleTool(2)} className={`scopeTopCell w-[25px] ${toolStates[2] ? "scopeToolOn" : ""}`}><ToolbarGlyph kind="dots" active={toolStates[2]}/></button>
        ...
        <div className="scopeTopGap w-[22px]"/><button onClick={() => toggleTool(4)} className={`scopeTopCell w-[48px] ${toolStates[4] ? "scopeToolOn" : ""}`}><ToolbarGlyph kind="filter" active={toolStates[4]}/></button><button onClick={() => toggleTool(5)} className={`scopeTopCell w-[50px] ${toolStates[5] ? "scopeToolOn" : ""}`}><ToolbarGlyph kind="route" active={toolStates[5]}/></button><div className="flex-1"/>
      </div>
      {showMenu && <div className="scopeMenu absolute left-[2px] top-[21px] z-[80] w-[205px] bg-[#064a40] text-[#e2e2e2]"><div className="scopeMenuTitle flex h-[12px] items-center justify-center border-b border-[#102f2a] text-[7px]">Title Menu</div><button onClick={() => { setShowScopeConfig(true); setShowMenu(false); }} className="block w-full border-b border-[#102f2a] px-[8px] py-[7px] text-left text-[12px] leading-none hover:bg-[#0a554a]">Scope configuration</button><div className="h-[118px]"/></div>}
      <div className="flex h-[23px] items-center border-t border-[#30383b] bg-[#555c61] pl-[27px] text-[10px] text-[#e3e3e3]"><span className="mr-[14px]">{stripTime}</span><button onClick={() => setShowChat((v) => !v)} className="mr-[12px]">CHATBOX</button><button onClick={() => showWindow("timer")} className="mr-[12px]">CLOCK</button><button onClick={() => showWindow("holds")} className="mr-[12px]">HOLDS</button><button onClick={() => showWindow("metar")} className="mr-[12px]">METAR</button><button>ATIS</button></div>
    </header>

    <section className="absolute inset-x-0 bottom-[112px] top-[44px] overflow-hidden bg-[#151515]">
      {windows.sector.open && <FloatingWindow windowKey="sector" state={windows.sector} title="SECTOR LIST" onDrag={startDrag} onClose={closeWindow} onCollapse={collapseWindow} onReset={resetWindow} className="w-[462px]"><div data-pf24-native-list-body="sector" /></FloatingWindow>}
      {windows.taxi.open && <FloatingWindow windowKey="taxi" state={windows.taxi} title="COMBINED TAXI LIST" onDrag={startDrag} onClose={closeWindow} onCollapse={collapseWindow} onReset={resetWindow} className="w-[190px]"><div data-pf24-native-list-body="taxi" /></FloatingWindow>}
      ...
      {windows.holds.open && <FloatingWindow windowKey="holds" state={windows.holds} title="HOLD LIST" onDrag={startDrag} onClose={closeWindow} onCollapse={collapseWindow} onReset={resetWindow} className="w-[232px]" simple><div className="border-x-2 border-b-2 border-[#ededed] bg-[#555c61] text-[14px] leading-[19px]"><div className="grid grid-cols-[28px_1fr_45px_45px] border-b border-[#ededed]"><span className="border-r border-[#ededed]"/><span className="text-center text-[17px]">CALLSIGN</span><span className="text-center text-[17px]">FL</span><span className="text-center text-[17px]">AFL</span></div><div className="grid h-[96px] grid-cols-[28px_1fr_45px_45px]"><span className="border-r border-[#ededed]"/><span/><span/><span/></div></div></FloatingWindow>}
      {windows.freq.open && <FloatingWindow windowKey="freq" state={windows.freq} title="Freq" onDrag={startDrag} onClose={closeWindow} onCollapse={collapseWindow} onReset={resetWindow} className="w-[134px]"><div data-pf24-native-list-body="freq" /></FloatingWindow>}
      {windows.metar.open && <FloatingWindow windowKey="metar" state={windows.metar} title="ATIS        Metars" onDrag={startDrag} onClose={closeWindow} onCollapse={collapseWindow} onReset={resetWindow} className="w-[330px]"><div className="max-h-[150px] overflow-y-auto px-1 py-1 text-[9px] leading-[13px] text-[#00efff]">{activeMetarAirports.length === 0 ? null : activeMetarAirports.map((station) => { const entry = metars[station]; return <div key={station} className="whitespace-nowrap"><span className="mr-2">X</span><span className="mr-2">{station}</span><span>{entry?.loading ? "LOADING..." : entry?.error ? "METAR UNAVAILABLE" : entry?.raw ? entry.raw.replace(new RegExp(`^${station}\\s*`, "i"), "") : "NO METAR"}</span></div>; })}</div></FloatingWindow>}
    ...
    {showChat && <footer className="absolute inset-x-0 bottom-0 z-40 h-[112px] bg-[#555c61] text-[9px]"><div className="h-[76px] overflow-hidden px-1 py-2 leading-[12px]">{consoleLines.map((line,index)=><div key={`${line}-${index}`}>{line}</div>)}</div><form className="flex h-[36px] items-center border-t border-[#777] bg-[#efefef] text-[#222]" onSubmit={(e)=>{e.preventDefault();executeCommand(command);}}><input value={command} onChange={(e)=>setCommand(e.target.value)} className="ml-16 h-[18px] w-[385px] bg-white px-1 outline-none"/><div className="ml-1 text-[8px]">{metarStation ? <>METAR&nbsp;&nbsp;{metarStation}&nbsp;&nbsp;{metarText.replace(metarStation, "").trim()}</> : null}</div></form></footer>}
...
function ScopeConfiguration({ zoom, setZoom, onClose }: { zoom:number; setZoom:React.Dispatch<React.SetStateAction<number>>; onClose:()=>void }) {
  const [tab,setTab]=useState<"general"|"personalization">("general"); const [draftZoom,setDraftZoom]=useState(zoom); useEffect(()=>setDraftZoom(zoom),[zoom]);
  const update=(v:number)=>setDraftZoom(Math.max(25,Math.min(175,Math.round(v)))); const cancel=()=>{setDraftZoom(zoom);onClose();}; const save=()=>{setZoom(draftZoom);onClose();};
  return <div className="absolute left-1/2 top-1/2 z-[90] h-[330px] w-[620px] -translate-x-1/2 -translate-y-1/2 border border-[#888] bg-[#cecece] font-mono text-[#111] shadow-[0_2px_10px_rgba(0,0,0,.5)]"><div className="flex h-[23px] items-stretch border-b border-white bg-[#cecece] text-[14px]"><button onClick={()=>setTab("general")} className={`flex items-center border-r border-white px-[12px] ${tab==="general"?"bg-[#d7d7d7]":""}`}>General</button><button onClick={()=>setTab("personalization")} className={`flex items-center border-r border-white px-[12px] ${tab==="personalization"?"bg-[#d7d7d7]":""}`}>Personalization</button></div><div className="h-[267px] px-[18px] pt-[30px] text-[14px]">{tab==="general"&&<div className="flex items-center gap-[14px]"><span>Default scope zoom</span><div className="relative flex h-[24px] w-[200px] items-center bg-[#efefef]"><input value={`${draftZoom}%`} readOnly className="scopeConfigField h-full w-full text-center text-[15px] outline-none"/><div className="absolute right-0 top-0 flex h-full w-[26px] border-l border-[#bcbcbc] bg-[#d9d9d9]"><button onClick={()=>update(draftZoom-5)} className="flex w-1/2 items-center justify-center border-r border-[#aaa] text-[9px]">▾</button><button onClick={()=>update(draftZoom+5)} className="flex w-1/2 items-center justify-center text-[9px]">▴</button></div></div></div>}</div><div className="absolute bottom-[10px] left-[12px] flex gap-[6px]"><button onClick={cancel} className="border border-[#9b9b9b] bg-[#e5e5e5] px-[14px] py-[3px]">Cancelar</button><button onClick={save} className="border border-[#9b9b9b] bg-[#e5e5e5] px-[14px] py-[3px]">Guardar</button></div></div>;
}
...
function RunwaySelector({ state, setState, onClose }: { state:RunwayState; setState:React.Dispatch<React.SetStateAction<RunwayState>>; onClose:()=>void }) {
  const toggle=(airport:string,runway:string,field:keyof RunwaySelection)=>{const key=`${airport}-${runway}`;setState((c)=>({...c,[key]:{...c[key],[field]:!c[key]?.[field]}}));}; const columns="grid-cols-[72px_68px_68px_1fr_1fr]";
  return <div className="absolute left-[390px] top-[70px] z-[95] w-[334px] max-w-[calc(100%-16px)] overflow-hidden border border-[#888] bg-[#cecece] font-mono text-[11px] text-[#111] shadow-[0_2px_8px_rgba(0,0,0,.45)]"><div className="flex h-[19px] items-center justify-between px-[4px]"><span>Runway selector dialog</span><button onClick={onClose} className="px-[4px]">×</button></div><div className={`grid ${columns} border-t border-white text-[12px]`}><div className="min-w-0 border-r border-white px-[4px] py-[2px]">Airport</div><div className="min-w-0 border-r border-white px-[4px] py-[2px] text-center">Active</div><div className="min-w-0 border-r border-white px-[4px] py-[2px] text-center">Runway</div><div className="min-w-0 border-r border-white px-[2px] py-[2px] text-center">DEP</div><div className="min-w-0 px-[2px] py-[2px] text-center">ARR</div></div><div className="max-h-[520px] overflow-y-auto overflow-x-hidden">{RUNWAYS.map(({airport,runway})=>{const key=`${airport}-${runway}`;const row=state[key]??{active:false,dep:false,arr:false};return <div key={key} className={`grid h-[20px] ${columns}`}><div className="min-w-0 border-r border-white px-[4px] leading-[20px]">{airport}</div><RunwayCell checked={row.active} onClick={()=>toggle(airport,runway,"active")}/><div className="min-w-0 border-r border-white px-[14px] leading-[20px]">{runway}</div><RunwayCell checked={row.dep} onClick={()=>toggle(airport,runway,"dep")}/><RunwayCell checked={row.arr} onClick={()=>toggle(airport,runway,"arr")}/></div>})}</div></div>;
}
...
function ToolbarGlyph({kind,active}:{kind:"screen"|"vector"|"dots"|"filter"|"route";active:boolean}) { const s=active?"#ffffff":"#e2e2e2"; const orange="#ff7433"; if(kind==="screen")return <svg width="21" height="19" viewBox="0 0 100 100"><rect x="30" y="7" width="40" height="78" fill="none" stroke={s} strokeWidth="7"/><text x="50" y="66" textAnchor="middle" fontSize="20" fontFamily="monospace" fill={s}>36</text><rect x="37" y="70" width="7" height="12" fill={s}/><rect x="49" y="70" width="7" height="12" fill={s}/><rect x="61" y="70" width="7" height="12" fill={s}/><rect x="73" y="70" width="7" height="12" fill={s}/></svg>; if(kind==="vector")return <svg width="22" height="19" viewBox="0 0 100 100"><rect x="13" y="51" width="37" height="37" fill="none" stroke={s} strokeWidth="7"/><line x1="59" y1="43" x2="73" y2="29" stroke={s} strokeWidth="7"/><line x1="81" y1="21" x2="94" y2="8" stroke={s} strokeWidth="7"/></svg>; if(kind==="dots")return <svg width="22" height="19" viewBox="0 0 100 100"><rect x="57" y="7" width="37" height="37" fill="none" stroke={s} strokeWidth="7"/><circle cx="13" cy="91" r="4.5" fill={s}/><circle cx="39" cy="85" r="4.5" fill={s}/><circle cx="64" cy="77" r="4.5" fill={s}/><circle cx="77" cy="61" r="4.5" fill={s}/></svg>; if(kind==="filter")return <svg width="41" height="19" viewBox="0 0 110 100"><polygon points="9,91 30,52 51,91" fill={s}/><line x1="45" y1="81" x2="74" y2="50" stroke={orange} strokeWidth="5"/><rect x="72" y="10" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="84.5" y1="42" x2="84.5" y2="55" stroke={s} strokeWidth="6"/><line x1="84.5" y1="64" x2="84.5" y2="77" stroke={s} strokeWidth="6"/></svg>; return <svg width="47" height="19" viewBox="0 0 120 100"><circle cx="16" cy="20" r="5" fill={s}/><rect x="37" y="10" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="53" y1="35" x2="72" y2="78" stroke={orange} strokeWidth="5"/><rect x="64" y="70" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="72" y1="20" x2="86" y2="20" stroke={s} strokeWidth="6"/><line x1="94" y1="20" x2="108" y2="20" stroke={s} strokeWidth="6"/><line x1="13" y1="82" x2="27" y2="82" stroke={s} strokeWidth="6"/><line x1="35" y1="82" x2="49" y2="82" stroke={s} strokeWidth="6"/><circle cx="109" cy="83" r="5" fill={s}/></svg>; }
