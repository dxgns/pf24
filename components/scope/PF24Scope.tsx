...
const DEFAULT_WINDOWS: Record<WindowKey, WindowState> = {
  ...
  holds: { x: 825, y: 58, open: true, collapsed: false }, freq: { x: 1120, y: 48, open: true, collapsed: false }, metar: { x: 1265, y: 48, open: true, collapsed: false },
};
...
export default function PF24Scope({ initialPlans }: Props) {
  ...
  useEffect(() => {
    setWindows((current) => { const next = { ...current }; (Object.keys(current) as WindowKey[]).forEach((key) => { const maxX = Math.max(0, logicalWidth - WINDOW_WIDTHS[key] - 4); const maxY = Math.max(44, logicalHeight - 170); next[key] = { ...current[key], x: Math.min(Math.max(0, current[key].x), maxX), y: Math.min(Math.max(44, current[key].y), maxY) }; }); return next; });
  }, [logicalWidth, logicalHeight]);
  useEffect(() => {
    const move = (e: MouseEvent) => { if (!dragRef.current) return; const { key, dx, dy } = dragRef.current; const mouseX = e.clientX / scopeScale; const mouseY = e.clientY / scopeScale; const maxX = Math.max(0, logicalWidth - WINDOW_WIDTHS[key] - 4); const maxY = Math.max(44, logicalHeight - 170); setWindows((cur) => ({ ...cur, [key]: { ...cur[key], x: Math.min(maxX, Math.max(0, mouseX - dx)), y: Math.min(maxY, Math.max(44, mouseY - dy)) } })); };
  ...
  return <main className="fixed left-0 top-0 overflow-hidden bg-[#151515] font-mono text-[12px] text-[#d8d8d8] select-none" style={{ width: `${100 / scopeScale}%`, height: `${100 / scopeScale}%`, transform: `scale(${scopeScale})`, transformOrigin: "top left" }}>
    <header className="absolute inset-x-0 top-0 z-50 h-[44px] border-b border-[#202426]">
      ...
      <div className="flex h-[23px] items-center border-t border-[#30383b] bg-[#555c61] pl-[27px] text-[10px] text-[#e3e3e3]"><span className="mr-[14px]">{stripTime}</span><button onClick={() => setShowChat((v) => !v)} className="mr-[12px]">CHATBOX</button><button onClick={() => setWindows((c) => ({...c,timer:{...c.timer,open:true}}))} className="mr-[12px]">CLOCK</button><button onClick={() => setWindows((c) => ({...c,holds:{...c.holds,open:true}}))} className="mr-[12px]">HOLDS</button><button onClick={() => setWindows((c) => ({...c,metar:{...c.metar,open:true}}))} className="mr-[12px]">METAR</button><button>ATIS</button></div>
    </header>
...
function ScopeConfiguration({ zoom, setZoom, onClose }: { zoom:number; setZoom:React.Dispatch<React.SetStateAction<number>>; onClose:()=>void }) {
  ...
  const update=(v:number)=>setDraftZoom(Math.max(25,Math.min(175,Math.round(v))));
  const cancel=()=>{setDraftZoom(zoom);onClose();};
  const save=()=>{setZoom(draftZoom);onClose();};
  return <div className="absolute left-1/2 top-1/2 z-[90] h-[330px] w-[620px] -translate-x-1/2 -translate-y-1/2 border border-[#888] bg-[#cecece] font-mono text-[#111] shadow-[0_2px_10px_rgba(0,0,0,.5)]">
    <div className="flex h-[23px] items-stretch border-b border-white bg-[#cecece] text-[14px]"><button onClick={()=>setTab("general")} className={`flex items-center border-r border-white px-[12px] ${tab==="general"?"bg-[#d7d7d7]":""}`}>General</button><button onClick={()=>setTab("personalization")} className={`flex items-center border-r border-white px-[12px] ${tab==="personalization"?"bg-[#d7d7d7]":""}`}>Personalization</button></div>
    <div className="h-[267px] px-[18px] pt-[30px] text-[14px]">{tab==="general"&&<div className="flex items-center gap-[14px]"><span>Default scope zoom</span><div className="relative flex h-[24px] w-[200px] items-center bg-[#efefef]"><input value={`${draftZoom}%`} readOnly className="scopeConfigField h-full w-full text-center text-[15px] outline-none"/><div className="absolute right-0 top-0 flex h-full w-[26px] border-l border-[#bcbcbc] bg-[#d9d9d9]"><button onClick={()=>update(draftZoom-5)} className="flex w-1/2 items-center justify-center border-r border-[#aaa] text-[9px]">▾</button><button onClick={()=>update(draftZoom+5)} className="flex w-1/2 items-center justify-center text-[9px]">▴</button></div></div></div>}</div>
    <div className="absolute bottom-[10px] left-[12px] flex gap-[6px]"><button onClick={cancel} className="border border-[#9b9b9b] bg-[#e5e5e5] px-[14px] py-[3px]">Cancelar</button><button onClick={save} className="border border-[#9b9b9b] bg-[#e5e5e5] px-[14px] py-[3px]">Guardar</button></div>
  </div>;
}
...
function ToolbarGlyph({kind,active}:{kind:"screen"|"vector"|"dots"|"filter"|"route";active:boolean}) {
  const s=active?"#ffffff":"#e2e2e2"; const orange="#ff7433";
  if(kind==="screen")return <svg width="21" height="19" viewBox="0 0 100 100"><rect x="30" y="7" width="40" height="78" fill="none" stroke={s} strokeWidth="7"/><text x="50" y="66" textAnchor="middle" fontSize="20" fontFamily="monospace" fill={s}>36</text><rect x="37" y="70" width="7" height="12" fill={s}/><rect x="49" y="70" width="7" height="12" fill={s}/><rect x="61" y="70" width="7" height="12" fill={s}/><rect x="73" y="70" width="7" height="12" fill={s}/></svg>;
  if(kind==="vector")return <svg width="22" height="19" viewBox="0 0 100 100"><rect x="13" y="51" width="37" height="37" fill="none" stroke={s} strokeWidth="7"/><line x1="59" y1="43" x2="73" y2="29" stroke={s} strokeWidth="7"/><line x1="81" y1="21" x2="94" y2="8" stroke={s} strokeWidth="7"/></svg>;
  if(kind==="dots")return <svg width="22" height="19" viewBox="0 0 100 100"><rect x="57" y="7" width="37" height="37" fill="none" stroke={s} strokeWidth="7"/><circle cx="13" cy="91" r="4.5" fill={s}/><circle cx="39" cy="85" r="4.5" fill={s}/><circle cx="64" cy="77" r="4.5" fill={s}/><circle cx="77" cy="61" r="4.5" fill={s}/></svg>;
  if(kind==="filter")return <svg width="41" height="19" viewBox="0 0 110 100"><polygon points="9,91 30,52 51,91" fill={s}/><line x1="45" y1="81" x2="74" y2="50" stroke={orange} strokeWidth="5"/><rect x="72" y="10" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="84.5" y1="42" x2="84.5" y2="55" stroke={s} strokeWidth="6"/><line x1="84.5" y1="64" x2="84.5" y2="77" stroke={s} strokeWidth="6"/></svg>;
  return <svg width="47" height="19" viewBox="0 0 120 100"><circle cx="16" cy="20" r="5" fill={s}/><rect x="37" y="10" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="53" y1="35" x2="72" y2="78" stroke={orange} strokeWidth="5"/><rect x="64" y="70" width="25" height="25" fill="none" stroke={s} strokeWidth="6"/><line x1="72" y1="20" x2="86" y2="20" stroke={s} strokeWidth="6"/><line x1="94" y1="20" x2="108" y2="20" stroke={s} strokeWidth="6"/><line x1="13" y1="82" x2="27" y2="82" stroke={s} strokeWidth="6"/><line x1="35" y1="82" x2="49" y2="82" stroke={s} strokeWidth="6"/><circle cx="109" cy="83" r="5" fill={s}/></svg>;
}
