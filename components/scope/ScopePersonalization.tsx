  ...
  try {
    ...
    return Object.fromEntries(
      Object.entries(DEFAULTS).map(([key, fallback]) => [
        key,
        normalizeColor(parsed[key as keyof Personalization], fallback),
      ]),
    ) as Personalization;
  } catch {
    return DEFAULTS;
...
function findPersonalizationHost(): HTMLElement | null {
  const button = Array.from(document.querySelectorAll<HTMLButtonElement>("button"))
    .find((item) => item.textContent?.trim() === "Personalization");
...
export default function ScopePersonalization() {
  ...
  const syncDialog = useCallback(() => {
    const nextHost = findPersonalizationHost();
    setHost(nextHost);
  }, []);
  ...
  useEffect(() => {
    const onClick = (event: MouseEvent) => {
      ...
      if (label === "Cancelar" || label === "Guardar") {
        setActive(false);
      }
    };
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      main.fixed > section > div.absolute.z-30 > div:first-child {
        background: var(--pf24-window-header, #555c61) !important;
        color: var(--pf24-window-text, #e9e9e9) !important;
      }
      ...
      [data-pf24-traffic-sim='true'] div[class*='text-[#00e000]'],
      [data-pf24-traffic-sim='true'] span[class*='text-[#00e000]'] { color: var(--pf24-traffic, #00e000) !important; }
  ...
  const fields: Array<[keyof Personalization, string]> = [
    ["radarBackground", "Radar background"],
    ["topBar", "Top bar"],
    ["secondaryBar", "Secondary bar"],
    ["windowHeader", "Window headers"],
    ["windowText", "Window header text"],
    ["traffic", "Traffic"],
    ["selectedTraffic", "Selected traffic"],
    ["vector", "HDG vector"],
    ["trail", "History trail"],
    ["metar", "METAR"],
  ];
  ...
  return createPortal(
    <div className="absolute inset-0 bg-[#cecece] px-[18px] pt-[18px] text-[12px] text-[#111]" data-pf24-personalization-panel="true">
          ...
          <label key={key} className="grid grid-cols-[1fr_92px] items-center gap-[8px]">
            ...
            <span className="flex h-[23px] items-center border border-[#aaa] bg-[#ededed] px-[3px]">
              <input
                ...
                className="h-[17px] w-[24px] cursor-pointer border-0 bg-transparent p-0"
              />
              <input
                ...
                className="ml-[4px] w-[58px] bg-transparent font-mono text-[10px] outline-none"
              />
            </span>
          </label>
        ))}
      ...
      <button
        type="button"
        onClick={reset}
        className="mt-[15px] border border-[#999] bg-[#e5e5e5] px-[10px] py-[3px] text-[11px]"
      >
        Restore defaults
      </button>
