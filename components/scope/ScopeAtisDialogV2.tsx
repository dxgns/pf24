...
type LatestAtis = { info_letter: string; created_at: string };
...
const COMBINED_RUNWAYS: Record<string, string[]> = { GCLP: ["03L/03R", "21L/21R"], MDPC: ["08/09", "26/27"] };
...
const TL_TABLE = [
  { min: 942.2, max: 959.4, ta3000: 60, ta4000: 70 }, { min: 959.5, max: 977.1, ta3000: 55, ta4000: 65 },
  { min: 977.2, max: 995.0, ta3000: 50, ta4000: 60 }, { min: 995.1, max: 1013.2, ta3000: 45, ta4000: 55 },
  { min: 1013.3, max: 1031.6, ta3000: 40, ta4000: 50 }, { min: 1031.7, max: 1050.3, ta3000: 35, ta4000: 45 },
] as const;

function emptyConfig(): AirportConfig { return { active: false, dep: "", arr: "", approach: "", remarks: "" }; }
function readConfigs(): ConfigMap { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}") as ConfigMap; } catch { return {}; } }
function writeConfigs(value: ConfigMap) { localStorage.setItem(STORAGE_KEY, JSON.stringify(value)); }
function nextLetter(letter?: string | null) { if (!letter || !/^[A-Z]$/.test(letter)) return "A"; return letter === "Z" ? "A" : String.fromCharCode(letter.charCodeAt(0) + 1); }
function previousLetter(letter?: string | null) { if (!letter || !/^[A-Z]$/.test(letter)) return "Z"; return letter === "A" ? "Z" : String.fromCharCode(letter.charCodeAt(0) - 1); }
function qnh(raw: string) { const q = raw.match(/\bQ(\d{4})\b/i)?.[1]; if (q) return Number(q); const a = raw.match(/\bA(\d{4})\b/i)?.[1]; return a ? (Number(a) / 100) * 33.8638866667 : null; }
function transitionLevel(airport: string, raw: string) { const value = qnh(raw); const ta = TA_BY_AIRPORT[airport]; if (value === null || !ta) return "---"; const band = TL_TABLE.find((item) => value >= item.min && value <= item.max); return band ? String(ta === 4000 ? band.ta4000 : band.ta3000).padStart(3, "0") : "---"; }
function metarCore(raw: string) { return raw.replace(/^METAR\s+/i, "").replace(/^SPECI\s+/i, "").trim(); }
...
async function latestAtis(airport: string): Promise<LatestAtis | null> { const { data } = await supabase.from("atis_messages").select("info_letter,created_at").eq("airport_icao", airport).order("created_at", { ascending: false }).limit(1).maybeSingle(); return data as LatestAtis | null; }
...
export default function ScopeAtisDialogV2({ controllerName }: { controllerName: string }) {
  ...
  const refreshLetters = useCallback(async () => {
    ...
    for (const row of data ?? []) if (!map[row.airport_icao]) map[row.airport_icao] = row.info_letter;
    setAtisLetters(map);
  }, []);
  ...
  const loadAirport = useCallback(async (icao: string) => {
    ...
    try {
      ...
      setInfoLetter(latest ? latest.info_letter : "A"); setMetar(""); setTransLvl("---");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const guardTopAtis = (event: MouseEvent) => {
      const button = event.target instanceof Element ? event.target.closest<HTMLButtonElement>("button[data-pf24-top-atis='true']") : null;
      if (!button) return;
  ...
  useEffect(() => {
    if (!connected) return;
    const check = async () => { for (const [icao, cfg] of Object.entries(readConfigs())) { try { if (await publishAutomatic(icao, cfg) && icao === airport) await loadAirport(icao); } catch (error) { console.error("PF24 automatic ATIS update failed", icao, error); } } };
    void check(); const timer = window.setInterval(() => void check(), 60_000); return () => window.clearInterval(timer);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (error) throw error; setMetar(raw); setTransLvl(tl); await refreshLetters(); setOpen(false);
    } catch (error) { console.error("PF24 ATIS publish failed", error); } finally { setBusy(false); }
  };

  if (!host || !open) return null;
  const preview = metar ? buildText(airport, infoLetter, metar, config, transLvl) : `${airport} ATIS INFO ${infoLetter}... METAR NO DISPONIBLE`;
      ...
      <div className="grid grid-cols-[305px_1fr] gap-[10px] px-[10px] pb-[12px]">
        <div>
          <div className="mb-[16px] flex items-center justify-between pt-[22px]">
            <label className="flex items-center gap-[8px] text-[16px]"><span>ATIS Active</span><button type="button" onClick={() => void setActive(!config.active)} className="relative h-[28px] w-[28px] overflow-hidden bg-[#efefef]">{config.active && <><span className="absolute left-[3px] top-[13px] h-[2px] w-[22px] rotate-45 bg-[#111]"/><span className="absolute left-[3px] top-[13px] h-[2px] w-[22px] -rotate-45 bg-[#111]"/></>}</button></label>
            <div className="flex items-center gap-[7px] text-[16px]"><span>ATIS Letter</span><button type="button" onClick={() => setInfoLetter((value) => previousLetter(value))} onContextMenu={(event) => { event.preventDefault(); setInfoLetter((value) => nextLetter(value)); }} title="Click izquierdo: letra anterior · Click derecho: letra siguiente" className="flex h-[28px] w-[28px] items-center justify-center bg-[#efefef] text-[24px]">{infoLetter}</button></div>
          </div>
          <div className="space-y-[9px]">
            ...
            <Field label="Runway DEP"><select value={config.dep} onChange={(e) => patch({ dep: e.target.value })} className="h-[23px] w-full bg-[#ececec] px-[4px] outline-none"><option value="" />{runwayOptions.map((value) => <option key={value}>{value}</option>)}</select></Field>
            <Field label="Runway ARR"><select value={config.arr} onChange={(e) => patch({ arr: e.target.value })} className="h-[23px] w-full bg-[#ececec] px-[4px] outline-none"><option value="" />{runwayOptions.map((value) => <option key={value}>{value}</option>)}</select></Field>
        ...
        <div className="min-w-0 overflow-hidden">
          ...
          <textarea value={config.remarks} maxLength={40} onChange={(e) => patch({ remarks: e.target.value.toUpperCase().slice(0, 40) })} className="mt-[3px] h-[100px] w-full resize-none overflow-x-hidden bg-[#ececec] p-[5px] uppercase outline-none [overflow-wrap:anywhere]" />
        </div>
      </div>
    </div>, host,
  );
...
function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="grid grid-cols-[145px_1fr] items-center gap-[7px] text-[16px]"><span className="text-right">{label}</span>{children}</label>; }
