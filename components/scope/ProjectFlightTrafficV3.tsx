...
export default function ProjectFlightTrafficV3({ initialPlans }: Props) {
  ...
  const dragLabelRef = useRef<{ id: string; dx: number; dy: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("scope-live-traffic-flight-plans").on(
      "postgres_changes",
    ...
    const open = () => {
      ...
      socket.onmessage = (event) => {
        void decodeMessage(event.data).then((decoded) => {
          ...
          const next = Array.from(liveRef.current.values()).map((value) => value.traffic).sort((a, b) => a.callsign.localeCompare(b.callsign));
          setTraffic(next);
    ...
    <div data-pf24-live-traffic="true" className="pointer-events-none absolute inset-0 z-[8] overflow-hidden">
      ...
      {traffic.map((item) => {
        ...
        const startLabelDrag = (event: React.MouseEvent<HTMLElement>) => {
          ...
          dragLabelRef.current = { id: item.id, dx: event.clientX - (host.getBoundingClientRect().left + labelPoint.x), dy: event.clientY - (host.getBoundingClientRect().top + labelPoint.y) };
        };
        ...
        return <div key={item.id}>
          ...
          {!active && <button
            ...
            className="pointer-events-auto absolute z-[9] w-[132px] cursor-move whitespace-nowrap text-left font-mono text-[#00e000]"
          ...
          >
            ...
            <span className="block pl-[80px] text-[12px] leading-[13px]">{destination}</span>
          </button>}

          {active && <div
            data-pf24-traffic-label="true"
            onMouseDown={startLabelDrag}
            className="pointer-events-auto absolute z-[12] w-[250px] cursor-move select-none font-mono text-[12px] leading-[17px] text-[#00e000]"
            style={{ left: labelPoint.x, top: labelPoint.y }}
          >
            <div className="text-[#ffff00]">A9999</div>
            <div className="grid grid-cols-[1fr_22px_70px] gap-x-1">
              <span>{item.callsign}</span><span>--</span><span>{item.aircraftType}</span>
            </div>
            <div className="grid grid-cols-[48px_1fr_58px] gap-x-1">
            ...
            <div className="grid grid-cols-[48px_48px_1fr] gap-x-1">
              <button
                type="button"
                onMouseDown={(event) => event.stopPropagation()}
            ...
            <div className="grid grid-cols-[78px_48px_1fr] gap-x-1">
              <button
                type="button"
                onMouseDown={(event) => {
                  event.stopPropagation();
